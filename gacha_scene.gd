...
const WARP_TYPES := [
	{
		...
		"tag":    "LIMITED",
		"icon":   "✦",
		"accent": Color(0.50, 0.40, 1.0),
		"banner_title": "Caelum Voss",
		"banner_sub":   "LIMITED",
		"art_icon":     "🔥",
		"art_col":      Color(1.0, 0.40, 0.15),
		"art_img":      "res://image/caelum_banner.png",
		"thumb_img":    "res://image/caelum_banner.png",
		"pool5":        ["Caelum Voss"],
	...
	},
	{
		...
		"tag":    "LIMITED",
		"icon":   "📖",
		"accent": Color(0.82, 0.58, 0.85),
		"banner_title": "Void Gauntlet",
		"banner_sub":   "LIMITED",
		"art_icon":     "📖",
		...
		"duration":     "อีก 21 วัน",
		"desc_lines": [
			"ทุก 10 ครั้งรับประกันได้อุปกรณ์ 4★ ขึ้นไป",
			"อุปกรณ์หลักในแบนเนอร์นี้มีอัตราได้รับสูงขึ้น",
		],
	},
]
...
func _build_hsr_ui() -> void:
	...
	# ── Character/equipment showcase art (right side) ──
	_rebuild_art()
...
func _rebuild_art() -> void:
	...
	art_rect.position     = Vector2((art_clip.size.x - disp_w) * 0.5, (art_clip.size.y - disp_h) * 0.78)
...
func _build_info_card() -> void:
	...
	title_clip.position = Vector2(pad, cy)
	title_clip.size = Vector2(iw - pad * 2, 52)
	title_clip.clip_contents = true
	title_clip.mouse_filter = Control.MOUSE_FILTER_IGNORE
	card.add_child(title_clip)
	var title := Label.new()
	...
	for desc_line in d.get("desc_lines", []) as Array:
		var dl_clip := Control.new()
		dl_clip.position = Vector2(pad, cy)
		dl_clip.size = Vector2(iw - pad * 2, 32)
		dl_clip.clip_contents = true
		dl_clip.mouse_filter = Control.MOUSE_FILTER_IGNORE
		card.add_child(dl_clip)
		var dl := Label.new()
...
func _on_warp_tab(idx: int) -> void:
	...
	_warp_btns.clear()
	_build_selector_strip()
	# Rebuild character/equipment showcase art
	_rebuild_art()
	# Rebuild info card
	_build_info_card()
	_refresh_ui()
